    ...
    <div className="bg-white text-slate-900">
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-12 px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {industries.map((industry, index) => (
              ...
              >
                <span className="text-xs font-semibold uppercase tracking-[0.3em] text-[#004da3]/70">
      ...
      {industries.map((industry, index) => (
        <SectionWrapper
          ...
          bg={index % 2 === 0 ? "bg-white" : "bg-[#004da3] text-white"}
          align="center"
        >
          <div className="mx-auto grid w-full max-w-6xl gap-10 px-4 sm:px-6 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1fr)] lg:items-center lg:gap-14 lg:px-8">
            <Reveal delay={index * 40} className="space-y-6">
              <span
                className={`text-xs font-semibold uppercase tracking-[0.3em] ${
                  index % 2 === 0 ? "text-[#004da3]/70" : "text-white/80"
                }`}
              >
                {String(index + 1).padStart(2, "0")} — Setor
              </span>
              <h3
                className={`text-3xl font-semibold leading-[1.38] tracking-[-0.02em] ${
                  index % 2 === 0 ? "text-[#07173d]" : "text-white"
                } sm:text-[2.6rem]`}
              >
                {industry.name}
              </h3>
              <p
                className={`text-base leading-relaxed ${
                  index % 2 === 0 ? "text-slate-600" : "text-white/80"
                }`}
              >
            ...
            <Reveal delay={120 + index * 40} className="rounded-[30px] border border-[#e1e8f8] bg-[#f5f8ff] p-8 shadow-[0_32px_80px_-60px_rgba(0,77,163,0.3)]">
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#004da3]/70">
                Soluções entregues
              </p>
              <div className="mt-5 flex flex-wrap gap-3">
                {info[industry.name]?.solutions.map((solution) => (
                  <span
                    ...
                    className="inline-flex items-center rounded-full border border-[#bcd5ff] bg-white px-4 py-2 text-[0.68rem] font-semibold uppercase tracking-[0.24em] text-[#004da3] transition-all duration-300 hover:-translate-y-1 hover:border-[#004da3] hover:bg-[#004da3] hover:text-white"
                  >
                    {solution}
                  </span>
                ))}
      ...
      <SectionWrapper theme="white" bg="bg-white">
        <div className="relative mx-auto max-w-4xl rounded-[36px] border border-[#dbe5f6] bg-white px-6 py-14 text-center shadow-[0_32px_96px_-68px_rgba(7,23,61,0.4)] sm:px-8 lg:px-12">
          <Reveal className="space-y-5">
            <span className="inline-flex items-center justify-center gap-2 rounded-full border border-[#004da3]/35 bg-[#f5f8ff] px-5 py-2 text-xs font-semibold uppercase tracking-[0.32em] text-[#004da3]/80">
              Próximo passo
            </span>
          ...
          <Reveal delay={160}>
            <Link
              to="/contato"
              className="btn-soft-elevate mt-10 inline-flex items-center justify-center gap-2 rounded-full bg-[#004da3] px-9 py-3 text-sm font-semibold text-white transition-transform hover:-translate-y-0.5"
            >
