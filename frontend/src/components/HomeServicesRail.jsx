...
const HomeServicesRail = () => {
  ...
  return (
    <section
      ...
      className="relative bg-white text-[#01233c]"
    >
      ...
      >
        <div className="space-y-6">
          <span className="inline-flex items-center gap-3 rounded-full border border-[#c6daf7] bg-[#f2f7ff] px-4 py-2 text-[0.65rem] font-semibold uppercase tracking-[0.3em] text-[#01233c]">
            Serviços
          </span>
          <div className="max-w-xl space-y-4">
            <h2 className="text-3xl font-semibold leading-[1.45] tracking-[-0.03em] text-[#01233c] sm:text-[3rem]">
              Impacto em cada etapa da jornada digital
            </h2>
            <p className="text-base leading-relaxed text-[#01233c]/80 sm:text-lg">
          ...
          >
            {services.map((service) => (
              ...
              >
                ...
                <div className="relative flex items-center justify-between text-xs font-semibold uppercase tracking-[0.28em] text-[#01233c]/60">
                  <span>{service.index}</span>
                  <span className="flex h-8 w-8 items-center justify-center rounded-full border border-[#c7dcff] text-[#01233c] transition group-hover:border-[#01233c] group-hover:bg-[#01233c]/10">
                    <svg
                      aria-hidden="true"
                ...
                <div className="relative space-y-2 text-[0.7rem] uppercase tracking-[0.22em] text-[#01233c]/60">
                  {service.features.slice(0, 3).map((feature, index) => (
                    <div key={`${service.slug}-${feature}-${index}`} className="flex items-center gap-3">
                      <span className="h-px flex-1 bg-[#d1dff3] transition-colors group-hover:bg-[#01233c]" />
                      <span>{feature}</span>
                    </div>
                  ))}
                </div>
